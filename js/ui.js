...
const UI = (function () {
  ...
  function renderNav(active) {
    ...
    const navLinks = el(
      ...
      links.map((l) =>
        el("a", { href: l.href, class: l.key === active ? "active" : "" }, [
          l.label,
        ]),
      ),
    );
      ...
      el("div", { class: "nav-inner container" }, [
        el("a", { href: "index.html", class: "brand" }, [
          ...
          el("span", { class: "brand-text" }, [
            ...
            el("small", {
              text: "International High School Baseball Analytics",
            }),
          ]),
        ]),
        navLinks,
  ...
  function renderFooter() {
    const f = el("footer", { class: "site-footer" }, [
      el("div", { class: "container" }, [
        el("p", {
          html: "International High School Baseball Analytics (IHBA) &middot; Static site (GitHub Pages) &middot; Data is stored in your browser.",
        }),
      ]),
    ]);
  ...
  function fmtMajorCell(type, val) {
    switch (type) {
      ...
      case "rate":
        return Stats.fmtRate(val);
      case "war":
        return AdvancedStats.fmtWar(val);
      case "repa":
        return AdvancedStats.fmtRePA(val);
      case "wrc":
        return AdvancedStats.fmtWRC(val);
      default:
  ...
  function majorRecordTable(rows) {
    ...
    while (i < MAJOR_RECORD_COLUMNS.length) {
      ...
      let j = i;
      while (
        j < MAJOR_RECORD_COLUMNS.length &&
        MAJOR_RECORD_COLUMNS[j].group === col.group
      )
        j++;
      row1.push(
        el("th", {
          colspan: String(j - i),
          class: "group-label",
          text: GROUP_LABELS[col.group] || "",
        }),
      );
      for (let k = i; k < j; k++)
        row2.push(el("th", { text: MAJOR_RECORD_COLUMNS[k].label }));
      i = j;
    }

    const thead = el("thead", {}, [
      el("tr", { class: "group-row" }, row1),
      el("tr", {}, row2),
    ]);
    const tbody = el(
      "tbody",
      ...
      rows.map((row) =>
        el(
          "tr",
          { class: row.Div === "Career" ? "career-row" : "" },
          MAJOR_RECORD_COLUMNS.map((col) =>
            el("td", {
              class:
                (col.type === "text" ? "" : "num") +
                (col.key === "Div" ? " div-cell" : ""),
